...
import torch
def objective(opt_vars, # 优化变量，即归一化后的末端执行器位姿
            og_bounds,
            keypoints_centered,
    ...
    # make sure z>0, robot arm does not hit the ground
    if opt_pose[2] < 0.0:
        cost += 1000.0
    ...
    if ik_result.success:
        ...
        reset_reg = 3.0
        # cost += 10  # mplib库无法得到ik计算次数，因此直接+10
    reset_reg_cost = 0.02 * reset_reg
    ...
    if is_grasp_stage:
        ...
        grasp_cost = -np.dot(opt_pose_homo[:3, 2], preferred_dir) + 1  # [0, 1] # panda 机器人的夹爪方向是 panda_hand 的 z 轴方向
        grasp_cost = 100.0 * grasp_cost
...
class SubgoalSolver:
    ...
    def _warmup(self):
        ee_pose = np.array([0.1, 0.0, 0.9, 0, 1, 0, -0.03])
    ...
    def _center_keypoints(self, ee_pose_homo, keypoints, keypoint_movable_mask):
        centering_transform = np.linalg.inv(ee_pose_homo)
        keypoints_centered = transform_keypoints(centering_transform, keypoints, keypoint_movable_mask)
    ...
    def solve(self,
        ...
        # = other setup
        # ====================================
        keypoints_centered = self._center_keypoints(ee_pose_homo, keypoints, keypoint_movable_mask)
        aux_args = (og_bounds,
                    keypoints_centered,
                    keypoint_movable_mask,
                    goal_constraints,
