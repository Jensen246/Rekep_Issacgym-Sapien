...
class Visualizer:
    ...
    def visualize_subgoal(self, subgoal_pose):
        ...
        ee_pose_nearby_points = generate_nearby_points(ee_pose[:3], num_points_per_side=6, half_range=0.003)
        ee_pose_nearby_colors = np.tile(self.color, (ee_pose_nearby_points.shape[0], 1))    # 绿色
        add_to_visualize_buffer(visualize_buffer, ee_pose_nearby_points, ee_pose_nearby_colors)
        
        subgoal_pose_nearby_points = generate_nearby_points(subgoal_pose[:3], num_points_per_side=6, half_range=0.003)
        subgoal_pose_nearby_colors = np.tile(np.array([1.0, 0.0, 0.0]), (subgoal_pose_nearby_points.shape[0], 1))    # 红色
        add_to_visualize_buffer(visualize_buffer, subgoal_pose_nearby_points, subgoal_pose_nearby_colors)
        # add keypoints
    ...
    def visualize_path(self, path):
        ...
        for t in range(path_length):
            ...
            # 生成路径点附近的点云，用于可视化
            pose_nearby_points = generate_nearby_points(path[t][:3], num_points_per_side=6, half_range=0.003)
            
            # 颜色渐变，从红色到白色
            base_color = np.array([1.0, 0.0, 0.0])  # 红色
            pose_color = (1 - whitening_coef) * base_color + whitening_coef * np.array([1, 1, 1])
            pose_nearby_colors = np.tile(pose_color, (pose_nearby_points.shape[0], 1))
            
            add_to_visualize_buffer(visualize_buffer, pose_nearby_points, pose_nearby_colors)
